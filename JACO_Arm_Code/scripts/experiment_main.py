...
faceExpModel = tf.keras.models.load_model("/home/project/Bjorn/ImageNet_CNN_binary/tf_keras_weights_thirdRevtf_keras_model_thirdRev.hdf5")
...
if __name__ == '__main__':
    ...
    def getAck(videoNum):
        ...
        facialExpression = intentionClassifier.intentClass(1, faceExpModel, faceCascade, detector, predictor, vidName)
    ...
    while True:
        ...
        if initialize == 1:
            ...
            headMovement = intentionClassifier.intentClass(2, faceExpModel, faceCascade, detector, predictor, vidName)
            ...
            if new_box_pos == 'X':
                ...
                endInteraction(old_box_pos)
                # Do endInteraction()
                break

            elif new_box_pos == 'S':
                print "Handing object to human"
                handObjecToHuman(old_box_pos)
                old_box_pos = new_box_pos
                print "Please place object back in position S"
                time.sleep(10)
                main_fingers(f_opening)
                ...
                # Add 1 due to getAck creates a video
                if (videoNum % 1) == 0:
                    videoNum += 1
                    acknowledgement = getAck(videoNum)
                    if acknowledgement:
                        thumbsUp()
                    else:
                        print old_box_pos
                        new_box_pos = 'C'
                        old_box_pos = pickAndPlace(old_box_pos, 'C')
                        #initialize = 0
